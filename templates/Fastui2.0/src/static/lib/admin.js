...
layui.define('view', function(exports) {
	...
	var events = admin.events = {
		...
		,iframeRefresh: function() {
			localStorage.setItem("title",$(this).parent().prev().text())
		...
		,internalJump:function(e){
			localStorage.setItem("title",e.context.dataset.text)
		}
		...
		,myApp: function() {
			admin.popupRight({
				...
				success: function() {
					view(this.id).render('popup/appNav');
				}
			});
		}
	...
	//面包屑
	element.on('nav(fastui-nav)', function(elem) {
		var crumbs='';
		$(elem).parents('.layui-nav-child').removeClass('layui-show');
		var crumbsNav = $(elem).closest('.layui-nav-item').children('a').find('cite').text().trim();
		var crumbsClass = $(elem).closest('div').prev().find("legend").text().trim();
		var crumbsMenu = $(elem).text().trim();
		if(crumbsClass==''&&crumbsNav !=''&&crumbsMenu!=''&&crumbsMenu!=crumbsNav){
			crumbs=crumbsNav+' / ' +crumbsMenu
		}else if(crumbsClass==''&& crumbsMenu==crumbsNav){
			crumbs=crumbsMenu
		}else{
			crumbs=crumbsNav + ' / '+crumbsClass+' / ' +crumbsMenu
		}
		localStorage.setItem("title",crumbs)
	})
