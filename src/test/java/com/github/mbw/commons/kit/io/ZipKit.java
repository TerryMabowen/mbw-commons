package com.github.mbw.commons.kit.io;
...
public class ZipKit {
    ...
    public static void extract(File zipFile, String dir) {
        if (zipFile == null) {
            throw new OCException("要解压的zip文件不能为空");
        }

        if (StringUtils.isBlank(dir)) {
            throw new OCException("解压后文件存放的目录不能为空");
        }

        if (zipFile.getPath().equals(dir)) {
            throw new OCException("压缩文件所在的目录不能和解压后文件存放的目录相同");
        }

        File fileDir = new File(dir);
        if (!fileDir.isDirectory()) {
            throw new OCException("dir不是一个目录：" + dir);
        }
        ...
        if (zipFile.getName().endsWith(".zip") || zipFile.getName().endsWith(".ZIP")) {
            try {
                ...
                throw new OCException("解压缩zip文件异常：" + exp.getMessage() + "，文件名：" + zipFile.getName(), exp);
            }
        } else {
            throw new OCException("被解压的文件不是以.zip或者.ZIP结尾的，传入的压缩文件是：" + zipFile.getName());
        }
    }
    ...
    public static String readByte2File(byte[] bytes, String dir, String zipName) {
        ...
        if (StringUtils.isBlank(zipName)) {
            throw new OCException("压缩文件名称不能为空");
        }
        ...
        if (zipName.endsWith(".zip") || zipName.endsWith(".ZIP")) {
            try {
                ...
                throw new OCException("将byte[]写入zip文件异常：" + exp.getMessage(), exp);
            }
        } else {
            throw new OCException("zipName必须以.zip或者.ZIP结尾，传入的zipName为：" + zipName);
        }
    }
    ...
    public static void compress(String dir, File[] files, String zipName) {
        if (StringUtils.isBlank(dir)) {
            throw new OCException("压缩文件保存的目录不能为空");
        }
        ...
        if (files.length <= 0) {
            throw new OCException("需要压缩的文件数组不能为空");
        }
        ...
        if (zipName.endsWith(".zip") || zipName.endsWith(".ZIP")) {
            try {
                ...
                throw new OCException("压缩文件异常：" + exp.getMessage(), exp);
            }
        } else {
            throw new OCException("zipName必须以.zip或者.ZIP结尾，传入的zipName为：" + zipName);
        }
